...
class Solver:
    ...
    def get_saveDis_simpleV_mileageCost_Fre(self, storagePrice_coe = 0.042, disPrice_coe = 5.2):
    ...
    def get_saveDisSto_simpleV_mileageStockCost_Fre(self, storagePrice_coe = 0.042, disPrice_coe = 5.2):
        ...
        for i in range(self.pro_num):
            ...
            for part in now_set:
                for j in range(part[6]):
                    tmp_per_left[duetime_map[part[6] - 1][j]] += self.car_num * part[5] * part[1] / 1000 * part[2] / \
                                                                 1000 * part[3] / 1000 / part[4] / part[6]
            for j in range(6):
                storage_timePlusV += tmp_per_left[j] * (duetime_point[j] - start_point)
            left.append(tmp_per_left)
        # print(left)
        ...
        for i in range(self.pro_num):
            ...
            while True:
                ...
                for j in range(self.pro_num):
                    for k in range(6):
                        if tag[j][k] == 0 and self.dis[now_point][j] < min_dis and t[now_point][j] > -1 and tmp_v > \
                                left[j][k] \
                                and due_time - start_point > tmp_time + avg_loading_time + self.dis[now_point][
                            j] / avg_v \
                                + self.my_data.get_dis(x1=self.machine_x, y1=self.machine_y, x2=self.x[j],
                                                       y2=self.y[j]) / avg_v:
                            min_dis = self.dis[now_point][j]
                            min_dis_point_j = j
                            min_dis_point_k = k
